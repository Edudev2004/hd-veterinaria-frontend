...
export const findMascotaInfo = (mascotaIdOrName: string | number): MascotaExpediente => {
  ...
  // 1. Buscar en pets.json
  const foundInMock = (petsMock as any[]).find(
    (p) => String(p.id).toLowerCase() === searchStr || p.name.toLowerCase() === searchStr
  );
  if (foundInMock) {
    return {
      id: String(foundInMock.id),
      nombre: foundInMock.name,
      especie: foundInMock.species || 'Canino',
      raza: foundInMock.breed,
      sexo: foundInMock.sex || 'macho',
      foto_url: foundInMock.image,
      propietario_id: 'prop-seed-01',
      propietario_nombre: 'Carlos Propietario'
    };
  }

  // 2. Buscar en localStorage mascotas si fue registrada dinámicamente
  try {
    const storedPets = JSON.parse(localStorage.getItem('vethd_db_mascotas') || '[]');
    const foundInStored = storedPets.find(
      (m: any) => String(m.id).toLowerCase() === searchStr || (m.nombre && m.nombre.toLowerCase() === searchStr)
    );
    if (foundInStored) {
      return {
        id: String(foundInStored.id),
        nombre: foundInStored.nombre,
        especie: foundInStored.especie || 'Canino',
        raza: foundInStored.raza,
        sexo: foundInStored.sexo,
        fecha_nacimiento: foundInStored.fechaNacimiento,
        foto_url: foundInStored.fotoUrl,
        propietario_id: foundInStored.propietarioId || 'prop-seed-01',
        propietario_nombre: 'Carlos Propietario'
      };
    }
  } catch {
...
export const getTodasLasMascotasConHistorial = (): {
  ...
  // 1. Añadir mascotas de pets.json
  (petsMock as any[]).forEach((p) => {
    mascotasMap.set(String(p.id), {
      id: String(p.id),
      nombre: p.name,
      especie: p.species || 'Canino',
      raza: p.breed,
      sexo: p.sex || 'macho',
      foto_url: p.image,
      propietario_id: 'prop-seed-01',
  ...
  // 2. Añadir mascotas creadas en localStorage si existen
  try {
    const storedPets = JSON.parse(localStorage.getItem('vethd_db_mascotas') || '[]');
    storedPets.forEach((p: any) => {
      mascotasMap.set(String(p.id), {
        id: String(p.id),
        nombre: p.nombre,
        especie: p.especie,
    ...
    // Continuar con mascotas del mock
  }
