...
export const ReviewsPage: React.FC = () => {
  const [appointments, setAppointments] = useState<AppointmentReview[]>(() => {
    ...
    return [
      { id: '1', petName: 'Max', service: 'Vacunación Anual', veterinarian: 'Dra. Ruiz', date: '10 de Septiembre, 2026', status: 'ATENDIDA' },
      { id: '2', petName: 'Luna', service: 'Vacunación Anual', veterinarian: 'Dra. Martínez', date: '15 de Septiembre, 2026', status: 'ATENDIDA' },
      { id: '3', petName: 'Max', service: 'Revisión Dental', veterinarian: 'Dra. Ruiz', date: '22 de Octubre, 2026', status: 'PENDIENTE' },
    ];
  });
  ...
  const handleSubmitRating = (e: React.FormEvent) => {
    ...
    setAppointments((prev) =>
      prev.map((app) =>
        app.id === selectedAppointment.id ? { ...app, rating, comment } : app
      )
    );
            ...
            <form onSubmit={handleSubmitRating} className="mt-6 space-y-4">
              <div className="flex justify-center space-x-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  ...
                  >
                    <Star className={`h-8 w-8 ${star <= (hover || rating) ? 'fill-amber-400 text-amber-400' : 'text-slate-300'}`} />
                  </button>
                ))}
              </div>
              ...
              <div className="flex justify-end space-x-3 pt-2">
                <button type="button" onClick={() => setIsModalOpen(false)} className="rounded-xl px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">
                  Cancelar
                </button>
                <button type="submit" disabled={rating === 0} className="rounded-xl bg-amber-500 px-4 py-2 text-sm font-medium text-white hover:bg-amber-600 disabled:opacity-50">
                  Enviar calificación
                </button>
              </div>
            </form>
